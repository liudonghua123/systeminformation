  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux) {
        ...
        exec('cat /etc/*-release; cat /usr/lib/os-release; cat /etc/openwrt_release; stat -c %w / | awk \'{print "INSTALL_DATE="$1}\'', function (error, stdout) {
          /**
           * @namespace
          ...
          result.installdate = release.INSTALL_DATE;
        });
      }
      if (_freebsd || _openbsd || _netbsd) {
      ...
      if (_windows) {
        ...
        try {
          const workload = [];
          workload.push(util.powerShell('Get-CimInstance Win32_OperatingSystem | select Caption,SerialNumber,BuildNumber,ServicePackMajorVersion,ServicePackMinorVersion,InstallDate | fl'));
          ...
          util.promiseAll(
            ...
            result.installdate = util.getValue(lines, 'InstallDate', ':').trim().split(/\s+/)[0];
          });
        } catch (e) {
          if (callback) { callback(result); }
          resolve(result);
